...
angular.module('myApp.mlb', ['ngRoute'])

.config(['$routeProvider', function($routeProvider) {
  $routeProvider.when('/mlb', {
    templateUrl: 'components/propbets/mlb/mlb_prop.html',
    controller: 'mlbCtrl'
  });
}])

.controller('mlbCtrl', function($scope) {
    $scope.$on('$viewContentLoaded', drawAllCharts());
})
...
drawChart("Strikeouts", [0,3,8], "Strikeouts", 1000, "chart_div_k");
    ...
    $.get("/data/baseballdata.csv", function(csvString) {
     ...
     // set chart options
		if (altMinY != 0) {
	        ...
	        vAxisOptions = {title: yLabel, viewWindow:{max:1400, min:1000}};
		} else {
